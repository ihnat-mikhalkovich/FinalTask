...
<head>
    ...
    <title><fmt:message key="editing_tariff" bundle="${ bundle }" /></title>
    <meta charset="utf-8">
...
<nav class="navbar navbar-inverse">
    <div class="container">
        ...
        <c:set var="currentPage" value="/FrontController?commandType=edit_tariff_form&tariffId=${ requestScope.extendedTariff.id }" />
...
<div class="container">
    <div class="jumbotron">
        <p class="text-center"><fmt:message key="editing_tariff" bundle="${ bundle }" /></p>
    </div>
    <form action="/FrontController" method="post">
        <div class="input-group">
            <input type="hidden" name="commandType" value="edit_tariff_save">

            <c:set var="extendedTariff" value="${ requestScope.extendedTariff }" scope="page" />

            <ul class="list-group">
                <li>
                    <span class="sign-up-text">ID: ${ extendedTariff.id }</span>
                    <input type="hidden" name="tariffId" value="${ extendedTariff.id }" />
                </li>
                <li><span class="sign-up-text"><fmt:message key="title" bundle="${ bundle }" />:</span>
                    <ul>
                        <c:forEach var="name" items="${ extendedTariff.names }" varStatus="status">
                            <li>
                ...
                <li><span class="sign-up-text"><fmt:message key="description" bundle="${ bundle }" />:</span>
                    <ul>
                        <c:forEach var="description" items="${ extendedTariff.descriptions }" varStatus="status">
                            <li>
                ...
                <li>
                    <div class="input-group sign-up-text">
                        <lable for="tariffDescription"><fmt:message key="cost" bundle="${ bundle }" />:</lable>
                        <input type="text" name="cost" id="tariffCost" class="form-control" value="${ extendedTariff.cost }">
                    </div>
                </li>
                <li>
                    <div class="input-group sign-up-text">
                        <lable for="visibility"><fmt:message key="visibility" bundle="${ bundle }" />:</lable>
                        <input type="checkBox" name="visibility" id="visibility" class="form-control" <c:if test="${ extendedTariff.visibility }">checked="checked"</c:if> />
                    </div>
                </li>
            </ul>
